...
public record RTTIReference(@NotNull Type type, @Nullable RTTIObject uuid, @Nullable String path) {
    ...
    @NotNull
    public FollowResult follow(@NotNull CoreBinary current, @NotNull PackfileManager manager, @NotNull RTTITypeRegistry registry) throws IOException {
        if (uuid == null) {
        ...
        if (path != null) {
            final List<Packfile> packfiles = manager.findAll(PackfileBase.getNormalizedPath(path));

            if (packfiles.isEmpty()) {
                throw new IllegalStateException("Unable to find referenced file: " + path);
            }

            if (packfiles.size() > 1) {
                throw new IllegalStateException("Multiple packfiles contain files under the same path: " + path);
            }

            binary = CoreBinary.from(packfiles.get(0).extract(PackfileBase.getNormalizedPath(path)), registry);
        } else {
            binary = current;
        }

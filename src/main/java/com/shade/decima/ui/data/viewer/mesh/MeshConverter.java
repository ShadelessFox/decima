...
public class MeshConverter {
    ...
    public DMFMesh processMesh() throws IOException {
        ...
        for (int j = 0; j < subMeshes.length; j++) {
            ...
            meshPrimitive.setIndexBuffer(indexBuffer, scene);
...
//                        copyBufferAsIs(blendWeightsName, meshPrimitive, weightBuffer.position(0), j);
                    }
                }
            }
    ...
    private void copyBufferAsIs(String attributeName, DMFPrimitive primitive, ByteBuffer data, int primitiveId) throws IOException {
        ...
        if (true) {
            String bufferFileName = resourceName + "_PRIMITIVE%d_".formatted(primitiveId) + descriptor.semantic + ".dbuf";
        ...
        vertexAttribute.componentType = descriptor.componentType.name();
        vertexAttribute.size = descriptor.elementType.getStride(descriptor.componentType);

        vertexAttribute.setBuffer(elementBuffer, scene);

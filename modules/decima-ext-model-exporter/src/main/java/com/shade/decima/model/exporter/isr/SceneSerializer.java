...
public class SceneSerializer {
    ...
    private static void serializeRegularSkinnedMeshResource(
        ...
        final Buffer buffer = new Buffer(object.obj("DataSource").<HwDataSource>cast().getData(project.getPackfileManager()));
        ...
        for (RTTIReference primitiveRef : primitives) {
            ...
            for (RTTIObject stream : vertexArray.objs("Streams")) {
                ...
                final BufferView view = new BufferView(
                    streamingVertices ? buffer : new Buffer(stream.get("Data")),
                    streamingVertices ? position : 0,
                    stride * vertexCount,
                    stride
                );
            ...
            final BufferView view = new BufferView(
                streamingIndices ? buffer : new Buffer(indexArray.get("Data")),
                streamingIndices ? position + startIndex * indexSize : 0,

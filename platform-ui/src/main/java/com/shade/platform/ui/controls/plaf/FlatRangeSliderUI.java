...
debug*/
    ...
    public void paintLabels(Graphics g) {
        FlatUIUtils.runWithoutRenderingHints(g, oldRenderingHints, () -> super.paintLabels(g));
    }
    ...
    public void paintTrack(Graphics g) {
        ...
        if (slider.getOrientation() == JSlider.HORIZONTAL) {
            ...
            if (enabled) {
                ...
                coloredTrack = new RoundRectangle2D.Float(cx, y, cw, tw, tw, tw);
            }
            track = new RoundRectangle2D.Float(trackRect.x, y, trackRect.width, tw, tw, tw);
        } else {
            float x = trackRect.x + (trackRect.width - tw) / 2f;
            if (enabled) {
                int cy = thumbRect2.y + (thumbRect2.height / 2);
                int ch = thumbRect.y - thumbRect2.y;
                coloredTrack = new RoundRectangle2D.Float(x, cy, tw, ch, tw, tw);
            }
            track = new RoundRectangle2D.Float(x, trackRect.y, tw, trackRect.height, tw, tw);
        }
